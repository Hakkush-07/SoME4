...
class Some4(VoiceoverScene):
    def construct(self):
        ...
        with self.voiceover(text="Take a circle on the plane.") as tracker:
        ...
        with self.voiceover(text="And take six points on it; A B C D E, and F.") as tracker:
            self.play(Create(A), Create(B), Create(C), Create(D), Create(E), Create(F), Create(VGroup(*tex_labels)))
        ...
        abcdefx = abcdef.copy()
        self.add(abcdefx)
        with self.voiceover(text="Connect AB and DE,") as tracker:
        ...
        with self.voiceover(text="and mark the intersection as X.") as tracker:
        ...
        abcdefy = abcdef.copy()
        self.add(abcdefy)
        with self.voiceover(text="Connect BC and EF,") as tracker:
        ...
        with self.voiceover(text="and mark the intersection as Y.") as tracker:
        ...
        abcdefz = abcdef.copy()
        self.add(abcdefz)
        with self.voiceover(text="Connect CD and FA,") as tracker:
        ...
        with self.voiceover(text="and mark the intersection as Z.") as tracker:
        ...
        with self.voiceover(text="These newly constructed points X, Y, Z are collinear, meaning they lie on the same line.") as tracker:
        ...
        with self.voiceover(text="This is independent of where the points are chosen on the circle, even the order doesn't matter.") as tracker:
        ...
        for str, v, angle in [("A", A, a), ("B", B, b), ("C", C, c), ("D", D, d), ("E", E, e), ("F", F, f)]:
            label = (MathTex(str).scale(0.75).move_to(v.get_center() + 0.5 * (RIGHT * cos(angle * pi / 180) + UP * sin(angle * pi / 180))))
            tex_labels.append(label)
        with self.voiceover(text="Again, take points A B C D E, and F on a circle.") as tracker:
            self.play(Create(unitcircle))
        ...
        x_text = MathTex(r"X=AB\cap DE \leftarrow").scale(0.5).move_to(abcdefx, LEFT).shift(2 * LEFT)
        with self.voiceover(text="And mark X, ") as tracker:
            self.play(Create(X), Create(x_label), Write(x_text), abcdefx[0:2].animate.set_fill(color=PURE_BLUE), abcdefx[3:5].animate.set_fill(color=PURE_GREEN))
        self.wait(0.5)
        ...
        y_text = MathTex(r"Y=BC\cap EF \leftarrow").scale(0.5).move_to(abcdefy, LEFT).shift(2 * LEFT)
        with self.voiceover(text="mark Y, ") as tracker:
            self.play(Create(Y), Create(y_label), Write(y_text), abcdefy[1:3].animate.set_fill(color=PURE_BLUE), abcdefy[4:6].animate.set_fill(color=PURE_GREEN))
        self.wait(0.5)
        ...
        z_text = MathTex(r"Z=CD\cap FA \leftarrow").scale(0.5).move_to(abcdefz, LEFT).shift(2 * LEFT)
        with self.voiceover(text=" and mark Z similarly. ") as tracker:
            self.play(Create(Z), Create(z_label), Write(z_text), abcdefz[0].animate.set_fill(color=PURE_GREEN), abcdefz[2:4].animate.set_fill(color=PURE_BLUE), abcdefz[5].animate.set_fill(color=PURE_GREEN))
        self.wait(1)
        ...
        with self.voiceover(text="They are on the same line again.") as tracker:
            self.play(Create(xyz))
        self.wait(1.5)
        pascal.add(xyz)
        ...
        with self.voiceover(text="This property is called Pascal's theorem, named after the famous mathematician Blaise Pascal. ") as tracker:
        ...
        with self.voiceover(text="The Latin name of this theorem is hexagrammum mysticum, ") as tracker:
            self.play(FadeIn(latin_text))
        self.wait(0.5)
        eng_text = Text("mystical hexagram", font_size=24, slant=ITALIC, color=GREEN).shift(DOWN * 1.5)
        with self.voiceover(text="meaning mystical hexagram.") as tracker:
        ...
        # possible proofs
        with self.voiceover(text="Today, we will prove this beautiful theorem.") as tracker:
            self.wait(1)
        ...
        with self.voiceover(text="There are different proofs using different techniques including") as tracker:
            self.wait(0.5)
        with self.voiceover(text="homothety,") as tracker:
            self.play(
            ChangeSpeed(
        ...
        with self.voiceover(text="menelaus theorem,") as tracker:
            self.play(
            ChangeSpeed(
        ...
        with self.voiceover(text="cross ratio,") as tracker:
            self.play(
            ChangeSpeed(
        ...
        with self.voiceover(text="isogonal conjugates,") as tracker:
            self.play(
            ChangeSpeed(
        ...
        with self.voiceover(text="and bezout's theorem, which we will choose to move forward.") as tracker:
            self.play(bezout_group.animate.scale(2.0))
        self.wait(0.5)
        with self.voiceover(text="This theorem concerns algebraic curves and their intersections.") as tracker:
            self.play(FadeOut(bezout_group), run_time=3.0)
        self.clear()
        self.wait(1)
        ...
        with self.voiceover(text="But first, we need to understand what an algebraic curve is.") as tracker:
            self.play(Write(title), run_time=2)
        self.wait(2)
        self.play(title.animate.shift(UP))

        curve = Tex(r"$\mathcal{C}=\{(x,y):P(x,y)=0\}$", font_size=30)
        with self.voiceover(text="Just like algebraic numbers are zeroes of polynomials in one variable, algebraic curves, which we will denote with fancy letters, are the zero sets of polynomials in two variables.") as tracker:
        ...
        with self.voiceover(text="The degree of a curve is naturally defined as the degree of the polynomial, which is the maximum number i+j such that we have a term x, to the power i; y, to the power j with a nonzero coefficient.") as tracker:
        ...
        with self.voiceover(text="Now, let's look at an example. Consider the polynomial y squared minus x cubed minus 3, which is of degree 3.") as tracker:
        ...
        self.wait(1)
        with self.voiceover(text="The graph of that curve looks like this.") as tracker:
            self.play(Create(elliptic_graph))
        self.play(FadeIn(number_plane))
        with self.voiceover(text="We will mostly rely on geometric intuition to get a grasp of Bezout's theorem.") as tracker:
            self.wait(1)
        self.wait(1)
        ...
        with self.voiceover(text="Let's see a few other curves.") as tracker:
            self.play(FadeOut(number_plane, shift=LEFT), FadeOut(elliptic_graph, shift=LEFT), FadeOut(elliptic_group, shift=LEFT))
        self.wait(0.5)
        with self.voiceover(text="The unit circle; x squared plus y squared minus 1, is a curve of degree 2.") as tracker:
        ...
        with self.voiceover(text="A line; for example y, minus 2x, minus 3, is a curve of degree 1.") as tracker:
            self.play(Write(line_group), FadeIn(number_plane, shift=LEFT), FadeIn(line_graph, shift=LEFT))
        self.wait(0.5)
        with self.voiceover(text="A little nuance is that we do not allow the same component to appear more than once, since that would contribute nothing to the zero set of a polynomial.") as tracker:
            self.wait(2)
        ...
        with self.voiceover(text="For example, we won't deal with polynomials like (y minus 2x minus 3)^2.") as tracker:
        ...
        with self.voiceover(text="However, multiplication of two different lines is perfectly fine, ") as tracker:
            self.play(FadeOut(number_plane, shift=LEFT), FadeOut(line_graph, shift=LEFT), FadeOut(*[line_group, line2_group], shift=LEFT))
        with self.voiceover(text="and indeed this is a curve of degree 2.") as tracker:
        ...
        with self.voiceover(text="y, minus 2x, minus 3,") as tracker:
        ...
        with self.voiceover(text="and 3y, minus x, plus 1,") as tracker:
            self.play(FadeIn(q_group, shift=RIGHT), FadeIn(q_graph))
        with self.voiceover(text="which are of degree 1,") as tracker:
            self.wait()

        pq = Dot([-2, -1, 0], radius=0.1)
        self.play(Create(pq))
        self.wait()
        with self.voiceover(text="They intersect at one point.") as tracker:
            self.play(FocusOn(pq))
        self.wait()

        with self.voiceover(text="In place of a line,") as tracker:
        ...
        circle_group.add(circle_box, circle)
        circle_graph = ImplicitFunction(lambda x, y: x ** 2 + y ** 2 - 1, color=YELLOW)
        with self.voiceover(text="put a circle, which is of degree 2,") as tracker:
        ...
        self.wait()
        with self.voiceover(text="they intersect at two points.") as tracker:
            self.play(FocusOn(u), FocusOn(v))
        self.wait(0.5)
        with self.voiceover(text="It may be less than that, but two intersections are the most we can get.") as tracker:
            self.wait(1)
        self.wait(1)

        with self.voiceover(text="In place of the another line,") as tracker:
        ...
        with self.voiceover(text="put this curve, y squared minus 8x cubed plus 3x minus 1, of degree 3") as tracker:
            self.play(FadeIn(elliptic_group, shift=RIGHT), FadeIn(elliptic_graph))
        with self.voiceover(text="now we have a curve of degree 2 and a curve of degree 3") as tracker:
            self.wait()
        ...
        self.wait()
        with self.voiceover(text="They intersect at 6 points.") as tracker:
        ...
        with self.voiceover(text="Finally, take these two curves of degree 3, y minus 3x cubed plus 3x") as tracker:
        ...
        with self.voiceover(text="and x minus 3y cubed plus 3y, ") as tracker:
        ...
        self.wait()
        with self.voiceover(text="they intersect at 9 points.") as tracker:
        ...
        with self.voiceover(text="These intuitive examples lead us to the Bezout's Theorem, ") as tracker:
            self.play(Write(title), run_time=2)
        self.wait(2)
        self.play(title.animate.shift(UP))
        with self.voiceover(text="which says that ") as tracker:
            self.play(Create(sep))

        poly_p = Tex(r"$$\mathcal{C}:\quad P(x,y)=\sum_{i+j\leq n}c_{i,j}x^iy^j$$", font_size=30, color=YELLOW)
        with self.voiceover(text="if we have a curve C of degree n, ") as tracker:
        ...
        with self.voiceover(text="and another curve D, of degree m, ") as tracker:
        ...
        pq_box = Rectangle()
        pq_box.surround(pq)
        with self.voiceover(text="then the number of intersections they have is at most nm, ") as tracker:
            self.play(Write(pq), Create(pq_box), run_time=2)
        with self.voiceover(text="the product of their degrees.") as tracker:
            self.wait(1)
        self.wait(0.5)
        with self.voiceover(text="An important note is that we require the curves not to have a common component, otherwise the theorem doesn't apply.") as tracker:
            self.wait(1)
        self.wait(1)
        with self.voiceover(text="We will now use this to prove Pascal's theorem.") as tracker:
        ...
        with self.voiceover(text="Let's construct the diagram of Pascal's theorem again.") as tracker:
            self.play(FadeIn(pascal))
        self.wait(2)
        shf = 3.5
        self.play(pascal.animate.shift(shf * RIGHT).scale(1.2))
        self.wait()
        ...
        cd.stroke_width = 3.0
        ef.stroke_width = 3.0
        with self.voiceover(text="Define C to be the product of the lines AB, CD, and EF, which has degree 3.") as tracker:
            self.play(ab.animate.set_color(YELLOW), cd.animate.set_color(YELLOW), ef.animate.set_color(YELLOW), Write(c))
        with self.voiceover(text="We will denote the degree of a curve with a subscript to the polynomial.") as tracker:
            self.wait(1)
        self.wait(2)
        ...
        de.stroke_width = 3.0
        fa.stroke_width = 3.0
        with self.voiceover(text="Similarly define D to be the product of lines BC, DE, and FA, which has also degree 3.") as tracker:
        ...
        with self.voiceover(text="And define E to be the circle.") as tracker:
        ...
        with self.voiceover(text="C and E intersect at A, B, C, D, E, F. again consistent with the Bezout's theorem.") as tracker:
            self.play(Write(ce))
        self.wait(2)
        self.play(Unwrite(ce))
        self.wait(1)
        de = Tex(r"$$\mathcal{D}\cap\mathcal{E}=\{A,B,C,D,E,F\}$$", font_size=30).shift(3 * LEFT + 1.0 * UP)
        with self.voiceover(text="The same goes for D and E.") as tracker:
            self.play(Write(de))
        self.wait(2)
        self.play(Unwrite(de))
        self.wait(1)
        g = 340
        G = Dot([radius * 1.2 * cos(g * pi / 180), radius * 1.2 * sin(g * pi / 180), 0]).shift(shf * RIGHT)
        g_label = MathTex("G").scale(0.75).scale(1.2).move_to(G.get_center() + 0.5 * (RIGHT * cos(g * pi / 180) + UP * sin(g * pi / 180)))
        with self.voiceover(text="Now take a point G on the circle, different from other points.") as tracker:
        ...
        f.align_to(c, LEFT)
        with self.voiceover(text="With the lambda defined as minus P of G over Q of G, we define the crucial curve F to be the curve P plus lambda times Q. Lambda is specifically chosen such that G lies on this curve. It has degree 3 because P and Q both have degree 3.") as tracker:
            self.play(Write(lmbd))
            self.wait()
            self.play(Write(f))
        self.wait()
        fe = Tex(r"$$\{A,B,C,D,E,F,G\}\subset\mathcal{E}\cap\mathcal{F}$$", font_size=30).shift(3 * LEFT + 0.0 * UP)
        fe.align_to(c, LEFT)
        with self.voiceover(text="Let's look at these 7 points A, B, C, D, E, F, G. They are all on the circle, which is of degree 2. And since F is constructed such that G is on it, and the other points are on both P and Q, these 7 points are all on the curve F as well. 7 is greater than 6, which is the product of the degrees of E and F. By Bezout's theorem, it seems impossible, but the theorem is valid only when the curves in discussion don't have a common component. Therefore, E and F must share a common component.") as tracker:
            self.play(Write(fe))
        self.wait(2)
        fet = Tex(r"$$S_3(x,y)=R_2(x,y)T_1(x,y)$$", font_size=30).shift(3 * LEFT + -0.5 * UP)
        fet.align_to(c, LEFT)
        with self.voiceover(text="That is, we can write polynomial S as the product of R and some other degree 1 polynomial, say T.") as tracker:
            self.play(Write(fet))
        self.wait(2)
        xyzf = Tex(r"$$X,Y,Z\in \mathcal{F}\implies S_3(X)=S_3(Y)=S_3(Z)=0$$", font_size=30).shift(3 * LEFT + -1.0 * UP)
        xyzf.align_to(c, LEFT)
        with self.voiceover(text="Points X, Y, Z are on curve F since they are all on P and Q.") as tracker:
            self.play(Write(xyzf))
        xyze = Tex(r"$$X,Y,Z\not\in \mathcal{E}\implies R_2(X)\neq 0,\;R_2(Y)\neq 0,\;R_2(Z)\neq 0$$", font_size=30).shift(3 * LEFT + -1.5 * UP)
        xyze.align_to(c, LEFT)
        self.wait(2)
        with self.voiceover(text="However, they are not on the circle.") as tracker:
            self.play(Write(xyze))
        self.wait(2)
        xyzt = Tex(r"$$T_1(X)=T_1(Y)=T_1(Z)=0$$", font_size=30).shift(3 * LEFT + -2.0 * UP)
        xyzt.align_to(c, LEFT)
        with self.voiceover(text="Hence, they must all lie on the curve defined by T, which is a line since it has degree 1.") as tracker:
            self.play(Write(xyzt))
        self.wait(2)
        with self.voiceover(text="And with that, we have proven Pascal's theorem.") as tracker:
            self.wait(3)
        ...
        self.wait()
        self.play(pascal.animate.shift(shf * LEFT))
        self.wait()
        pascal.remove(unitcircle)
        self.add(unitcircle)
        with self.voiceover(text="One thing you might have noticed is that, in this proof, we only used the fact that E is of degree 2, not that it is a circle.") as tracker:
            self.play(FadeOut(pascal))
        with self.voiceover(text="Therefore, this theorem is valid for any curve of degree 2.") as tracker:
            self.play(FadeIn(pascal))
        with self.voiceover(text="In fact, what we have seen so far is only a special case of Pascal's theorem, which is generally stated for any curve of degree 2. We will look at one more interesting case.") as tracker:
            self.wait(1)
        self.play(FadeOut(pascal))
        ...
        with self.voiceover(text="where the curve we choose our points on is the product of two lines.") as tracker:
            self.play(Transform(unitcircle, two_lines))
        self.wait()
        title = Text("Pappus's Theorem", font_size=30)
        with self.voiceover(text="It even has its own name, Pappus's theorem.") as tracker:
        ...
        with self.voiceover(text="Similarly, we take points A, B, C, D, E, and F on two lines.") as tracker:
            self.play(Create(A), Create(B), Create(C), Create(D), Create(E), Create(F))
            self.wait(1)
            self.play(Create(VGroup(*tex_labels)))
        self.wait()
        ...
        ab = Line(A.get_center(), B.get_center())
        de = Line(D.get_center(), E.get_center())
        with self.voiceover(text="Construct X.") as tracker:
        ...
        bc = Line(B.get_center(), C.get_center())
        ef = Line(E.get_center(), F.get_center())
        with self.voiceover(text="Construct Y.") as tracker:
        ...
        cd = Line(C.get_center(), D.get_center())
        fa = Line(F.get_center(), A.get_center())
        with self.voiceover(text="And construct Z.") as tracker:
        ...
        xyz = DashedLine(X.get_center(), Y.get_center())
        pappus.add(xyz)
        with self.voiceover(text="Points X, Y, and Z are on the same line again!") as tracker:
            self.play(Create(xyz))
        self.wait(2)
        ...
        with self.voiceover(text="With that, we conclude this video. Hope you enjoyed. Thank you, and see you again!") as tracker:
